class FakeWebSocket {
    constructor(url) {
        ...
        Main.lossLimitRefreshDuration = Main.lossLimitRefreshDuration / TestModel.speed;
        Volatility.duration = Volatility.duration / TestModel.speed;
        Main.breakExtention = Main.breakExtention / TestModel.speed;
        Util.intervalDuration = Util.intervalDuration / TestModel.speed;
    ...
    messageBack(key, data, all) {
        ...
        switch (key) {
            ...
            case 'proposal':
                ...
                //console.log('proposal',all.amount);
        ...
        this.tickInterval = setInterval(() => {
            ...
            if (TestModel.isBuy && !TestModel.purchasedTick) {
                ...
                if (TestModel.transactionCount >= TestModel.tickDuration) {
                    let result = TestModel.getResult();

                    if (result !== '0.00') TestModel.balance += TestModel.currentStake + (TestModel.currentStake * 0.94);
                    this.message('balance', {
                        balance: TestModel.balance
                    });
                    this.message('transaction', {
